...
const Navbar = () => {
  ...
  return (
    <div className="lg:hidden">
      {!isOpen ? (
        <div className="h-14 px-3.5 bg-[#faf8f7] fixed bottom-0 left-0 right-0 z-50 border-t-4 border-[#f7f3f1] flex justify-between items-center">
          <div className="text-[14px]">Home</div>
          <IoMdMenu size={28} onClick={() => setIsOpen(true)} />
        </div>
      ) : (
        <div>
          <div className="h-[210px] px-2.5 bg-[#faf8f7] fixed bottom-0 left-0 right-0 z-50 border-t-4 border-[#f7f3f1] rounded-3xl">
